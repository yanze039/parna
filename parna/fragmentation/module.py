...
import numpy as np
from parna.utils import merge_list, flatten_list, map_atoms
...
class Fragmentizer:
    ...
    def get_rotatable_bonds(self, non_14_hydrogen: bool = True):
    ...
    def fragmentize(self, non_14_hydrogen: bool = True):
        bonds = self.get_rotatable_bonds(non_14_hydrogen=non_14_hydrogen)
        logger.info(f"Found {len(bonds)} rotatable bonds.")
        self.fragments = self.fragment_on_bonds(bonds)
    ...
    def _get_fragment_atom_mapping(self):
        ...
        for fragment in self.fragments:
            self.fragment_atom_mapping.append(self.mapping_atoms(fragment))
    ...
    def mapping_atoms(self, fragment):
        atom_mapping = map_atoms(self.mol, fragment)
        return atom_mapping
        
    def get_fragments(self, non_14_hydrogen: bool = True):
        if hasattr(self, 'fragments'):
            return self.fragments
        else:
            self.fragmentize(non_14_hydrogen=non_14_hydrogen)
            return self.fragments
