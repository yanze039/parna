...
def gen_topology(seq1, seq2):
    ...
    for pos in range(len(seq2_comp)):
        ...
        if pos == 0:
            seq2_res_mol2 = Path(lib_cap/f"cap/{seq2_res}.lib/{seq2_res}.mol2")
        elif pos == 1:
            seq2_res_mol2 = Path(lib_linker/f"linker/{seq2_res}.lib/{seq2_res}.mol2")
        elif pos == 2:
            seq2_res_mol2 = Path(lib_na/f"nucleotides/{seq2_res}.lib/{seq2_res}_without_phosphate.mol2")
            seq2_res_lib = Path(lib_na/f"nucleotides/{seq2_res}.lib/{seq2_res}.lib")
        else:
        ...
        if pos == 0:
            seq1_res_lib = Path(lib_cap/f"cap/{seq1_res}.lib/{seq1_res}.lib")
            seq2_res_lib = Path(lib_cap/f"cap/{seq2_res}.lib/{seq2_res}.lib")
            ...
            seq1_res_lib = Path(lib_linker/f"linker/{seq1_res}.lib/{seq1_res}.lib")
            seq1_res_pmd_mol = parmed.load_file(str(seq1_res_lib))[seq1_res]
            seq1_tail_idx = seq1_res_pmd_mol.tail.idx
            seq1_head_idx = seq1_res_pmd_mol.head.idx
            
            seq2_res_lib = Path(lib_linker/f"linker/{seq2_res}.lib/{seq2_res}.lib")
            seq2_res_pmd_mol = parmed.load_file(str(seq2_res_lib))[seq2_res]
            seq2_tail_idx = seq2_res_pmd_mol.tail.idx
            seq2_head_idx = seq2_res_pmd_mol.head.idx
            ...
            pdb_block_complex = build_residue_pdb_block(
                ...
                residue_tail_idx=seq2_tail_idx, 
                template_tail_idx=seq1_tail_idx,
                residue_head_idx=seq2_head_idx, 
                template_head_idx=seq1_head_idx,
                atomCompare=Chem.rdFMCS.AtomCompare.CompareAnyHeavyAtom,
                only_heavy_atoms=True
            )
            pdb_block_ligands = build_residue_pdb_block(
    ...
    for i in range(len(seq1_comp)):
        if i == 0:
            new_patch.append(lib_cap / f"cap/{seq1_comp[i]}.lib/{seq1_comp[i]}.lib")
            new_patch.append(lib_cap / f"cap/{seq2_comp[i]}.lib/{seq2_comp[i]}.lib")
        elif i == 1:
            new_patch.append(lib_linker / f"linker/{seq1_comp[i]}.lib/{seq1_comp[i]}.lib")
            new_patch.append(lib_linker / f"linker/{seq2_comp[i]}.lib/{seq2_comp[i]}.lib")
        elif i == 2:
