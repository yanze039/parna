...
import parmed
import parna
from parna.utils import map_atoms, rd_load_file, atomName_to_index
...
lib_dir = Path("/home/gridsan/ywang3/Project/Capping/Benchmarking/lib")
data_dir = Path("/home/gridsan/ywang3/Project/Capping/Benchmarking/data")
...
data_dir_cap = Path("param_refine_1010_cap_manual")
data_dir_na  = Path("data")

sugar_template_file = "/parna/template/sugar_template.pdb"
lib_dir = Path("lib")
sugar_tmpl_mol = rd_load_file(sugar_template_file)
...
for i in range(n_residues_alchem):
    ...
    if "zeta_alpha" in parm_info:
        ...
        found_zeta_alpha = False
        for C5_name in ["C5'", "C01", "C5"]:
            try:
                alpha_atom_names = _alpha_atom_names + ["P", "O5'", C5_name]
                pmd_mol = patch_cmap_parameters(
                    pmd_mol, cmap_grid, 24, i, residue_mol,
                    zeta_atom_names, alpha_atom_names, connection_shifts=[1,1]
                )
                found_zeta_alpha = True
                break
            except Exception as e:
                found_zeta_alpha = False
                continue
        if not found_zeta_alpha:
            print(f"can't find the zeta alpha dihedral for {resname} at {i}")
            raise ValueError("can't find the zeta alpha dihedral")
...
for item in ["CHARMM_CMAP_COUNT", "CHARMM_CMAP_RESOLUTION", "CHARMM_CMAP_INDEX"]:
